...
type RawSubmission = Prisma.AssignmentSubmissionGetPayload<{
...
export class AssignmentSubmissionsService {
  ...
  async findLatestForEnrollment(enrollmentId: number) {
    ...
    for (const row of rows) {
      if (!latestByLecture.has(row.lecture_id))
        latestByLecture.set(row.lecture_id, row);
    }

    return {
      success: true,
      data: Array.from(latestByLecture.values()).map(mapSubmission),
    };
  }
  ...
  async reviewSubmission(submissionId: bigint, dto: ReviewSubmissionDto) {
    ...
    const isResolved =
      dto.submission_status === 'REVIEWED' ||
      dto.submission_status === 'RESUBMISSION_REQUIRED';
  ...
  private async notifyTrainers(
    ...
    const trainerUserIds = [
      ...new Set(assignments.map((a) => a.trainer.user_id)),
    ];
