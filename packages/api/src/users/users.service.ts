...
export class UsersService {
    ...
    try {
      ...
      if (resolvedRoleName) {
        const role = await this.prisma.role.findFirst({
          where: {
            role_name: { equals: resolvedRoleName, mode: 'insensitive' },
          },
        });
        if (!role) {
          await this.prisma.user.delete({ where: { user_id: user.user_id } });
          throw new BadRequestException(
            `Role '${resolvedRoleName}' does not exist`,
          );
        }
  ...
  async findAll(queryUserDto: QueryUserDto) {
    ...
    const where: Prisma.UserWhereInput = {
      ...
      ...(institutionId && {
        studentProfile: { institution_id: Number(institutionId) },
      }),
    };
  ...
  async update(id: string, updateUserDto: UpdateUserDto) {
    ...
    if (status) data.account_status = status;
