...
describe('StudentProfilesController', () => {
      ...
      providers: [
        {
          ...
          useValue: mockStudentProfilesService,
        },
      ],
    }).compile();
  ...
  it('create — delegates to service with userId, dto, and admin id', async () => {
    ...
    mockStudentProfilesService.create.mockResolvedValue({
      success: true,
      data: {},
    });
    await controller.create(3, dto, mockUser);
    expect(mockStudentProfilesService.create).toHaveBeenCalledWith(
      3,
      dto,
      mockUser.sub,
    );
  ...
  it('findByUser — passes userId to service', async () => {
    mockStudentProfilesService.findByUser.mockResolvedValue({
      success: true,
      data: {},
    });
  ...
  it('findAll — delegates to service with query', async () => {
    mockStudentProfilesService.findAll.mockResolvedValue({
      success: true,
      data: [],
      meta: {},
    });
    await controller.findAll({ page: 1, limit: 10 });
    expect(mockStudentProfilesService.findAll).toHaveBeenCalledWith({
      page: 1,
      limit: 10,
    });
  });
  ...
  it('findOne — passes parsed int id to service', async () => {
    mockStudentProfilesService.findOne.mockResolvedValue({
      success: true,
      data: {},
    });
  ...
  it('update — passes id, dto, and admin id to service', async () => {
    mockStudentProfilesService.update.mockResolvedValue({
      success: true,
      data: {},
    });
    await controller.update(1, { department: 'CS' }, mockUser);
    expect(mockStudentProfilesService.update).toHaveBeenCalledWith(
      1,
      { department: 'CS' },
      mockUser.sub,
    );
  ...
  it('remove — passes id to service', async () => {
    mockStudentProfilesService.remove.mockResolvedValue({
      success: true,
      message: 'deleted',
    });
